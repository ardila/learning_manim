...
num_circumferences = 10

def get_internal_circumferences():
    return [Circle(radius=(1 - i / num_circumferences), color=TEAL).set_stroke(width=3) for i in range(1, num_circumferences+1)]
...
class Matike(Scene):
    ...
    def unroll_circumferences(self):
        ...
        for i, circumference in l:
            i = i + 1
            unrolled = Line(
                (0, -0.5 + i / num_circumferences, 0), ((0 + circumference.radius * TAU, -0.5 + i / num_circumferences, 0)), stroke_width=3).set_color(TEAL)
